...
try {
  ...
  class NoLogError extends Error {
    ...
    constructor(originalError: unknown, message: string) {
      super(
        NoLogError.isError(originalError) ? originalError.message : message
      );
      ...
      if (stack) {
        ...
        for (const line of lines) {
          ...
          if (match) {
            ...
            const mapped = sourceMapReader.getOriginalPosition(
              +line!,
              +column!
            );
            if (mapped) {
  ...
  class TestRunner {
    ...
    constructor(
      private testFileQueue: TestUnit[],
    ...
    async runTestCase(
      ...
      try {
        ...
        output.info.print(
          html`<pre>    [✘] <span color="lightRed">${raw(
            testPath
          )}</span></pre>`
        );
        if (_isExpectError(e)) {
